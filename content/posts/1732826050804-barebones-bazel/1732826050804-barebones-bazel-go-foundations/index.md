...
3. **Create the Currency Module:** Create the directory `currency` inside `go-level-up`. Open your terminal, navigate to your module directory (`go-level-up/currency`), and create the module using the following command:

    ```bash
    bazel run @rules_go//go:go -- mod init github.com/bochap-learning/barebones-bazel/go-level-up/currency
    ```
...
4. **Create the Currency Logic:** Create the file `converter.go` inside `go-level-up/currency`
...
5. **Create the Currency Tests:** Create the file `converter_test.go` inside `go-level-up/currency`
...
6. **Create the Client Module:** Create the directory `client` inside `go-level-up`. Open your terminal, navigate to your module directory (`go-level-up/client`), and create the module using the following command:

    ```bash
    bazel run @rules_go//go:go -- mod init github.com/bochap-learning/barebones-bazel/go-level-up/client
    ```

    > **_NOTE:_** To avoid potential inconsistencies between your project's Go version and the one used by Bazel, use the Go SDK that Bazel installs.

7.  **Create the Main:** Create the file `main.go` inside `go-level-up/client` with the following content:
...
8. **Create the Currency Library `BUILD` File:** Create the file `BUILD.bazel` inside `go-level-up/currency`
...
9. **Build the Currency Library:** Open your terminal, navigate to your module directory (`go-level-up`), and run the following command:
 
    ```bash
    bazel build //go-level-up/currency:converter
    ```

10. **Run the Currency Tests:** Open your terminal, navigate to your module directory (`go-level-up`), and run the following command:

    ```bash
    bazel test //go-level-up/currency:converter_test --cache_test_results=no
    ```

11. **Create the Currency Client `BUILD` File:** Create the file `BUILD.bazel` inside `go-level-up/client`
...
12. **Build the Currency Client:** Open your terminal, navigate to your module directory (`go-level-up`), and run the following command:
...
16.  **Changes made:** The changes made for Go Level Up change be found in the commit for commit [0cd141](https://github.com/bochap-learning/barebones-bazel/commit/0cd141d47c882ef216f23f350082352ad78ed5bf)
